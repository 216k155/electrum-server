...
class BCDataStream(object):
    """Workalike python implementation of Lux's CDataStream class."""
    def __init__(self):
        self.input = None
        self.read_cursor = 0
    ...
    def read_string(self):
        ...
        # ... and the Lux client is coded to understand:
...
# This function comes from lux tools, bct-LICENSE.txt.
def long_hex(bytes):
...
def parse_TxIn(vds):
    ...
    if scriptSig:
        pubkeys, signatures, address = get_address_input_script(scriptSig)
    else:
        pubkeys = []
        signatures = []
        address = None
...
def get_address_from_output_script(bytes):
    ...
    # Pay-by-Lux-address TxOuts look like:
    # DUP HASH160 20 BYTES:... EQUALVERIFY CHECKSIG
